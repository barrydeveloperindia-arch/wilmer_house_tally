...
function parseDateFromFilename(filename) {
    ...
    return "20250513"; // fallback to end of statement period
}
...
async function main() {
    const csv = fs.readFileSync('Extracted_CC_Jan_May_2025.csv', 'utf8');
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (cols.length >= 4) {
            rows.push({
                ...
                desc: cols[2].replace(/"/g, '').trim(), // CleanMerchant
    ...
    console.log(ledgersResponse.match(/<(CREATED|ERRORS|EXCEPTIONS|LINEERROR)>[^<]+<\/(CREATED|ERRORS|EXCEPTIONS|LINEERROR)>/g)?.join('\\n') || ledgersResponse.substring(0, 300));
